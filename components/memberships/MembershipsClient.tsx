  ...
  return (
    <main>
      ...
      <section className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {tiers.map((tier) => (
          <div key={tier.id} className={`relative flex flex-col border-neutral-200 ${tier.popular ? "ring-2 ring-emerald-600" : ""}`}>
            {tier.popular && (
            ...
            <div className="p-4">
              {tier.ctaHref.startsWith("/") ? (
                <Link href={tier.ctaHref} className="w-full">
                  <Button className="w-full bg-emerald-700 hover:bg-emerald-800 text-white">{tier.cta}</Button>
                </Link>
              ) : (
